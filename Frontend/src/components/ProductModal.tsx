...
export default function ProductModal({
  ...
  // если product не передали — берём по slug из URL
  const slugFromUrl = params?.slug;
  const effectiveProduct: Product | null =
    product ??
    (slugFromUrl
      ? products.find((p) => p.slug === slugFromUrl) ?? null
      : null);
  ...
  const imageSrc = effectiveProduct.image.startsWith('/')
  ...
  const handleApply = () => {
    ...
    try {
      localStorage.setItem('orderData', JSON.stringify(cfg));
    } catch {}
  ...
  return (
    <>
      <Helmet>
        {/* SEO META */}
        <title>
          {effectiveProduct.name} — купить корзину для кондиционера
        </title>
        <meta
          name="description"
          content={descriptionText.slice(0, 160)}
        />
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {effectiveProduct.specifications.material}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Отделка:</span>
                  <span className="font-medium">
                    {effectiveProduct.specifications.finish}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Покрытие:</span>
                  <span className="font-medium">
                    {effectiveProduct.specifications.coating}
                  </span>
                </div>
              </div>
            </div>
